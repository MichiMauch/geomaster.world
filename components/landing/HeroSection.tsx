...
export default function HeroSection({ locale }: HeroSectionProps) {
  ...
  // Unauthenticated: Globe as fullscreen background
  return (
      ...
      <motion.div className="container max-w-6xl mx-auto px-4" style={{ y }}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="text-center lg:text-left">
            {/* Logo/Title */}
            <h1 className="text-4xl md:text-5xl lg:text-6xl mb-4" style={{ letterSpacing: "-1px" }}>
              <span className="text-foreground font-extrabold">GeoMaster</span>
              <span className="font-light" style={{ color: "#FF6B35" }}> World</span>
            </h1>

            {/* Subtitle */}
            <p className="text-lg md:text-xl text-white mb-3 max-w-md mx-auto lg:mx-0">
              {t("hero.subtitle")}
            </p>

            {/* Tagline */}
            <p className="text-lg md:text-xl text-white/90 mb-6 max-w-md mx-auto lg:mx-0">
              {t("hero.tagline")}
            </p>

            {/* Stats Bar - Dynamic Quiz Counts */}
            <div className="flex flex-wrap justify-center lg:justify-start items-center gap-3 sm:gap-4 mb-8">
              <div className="flex items-baseline gap-1">
                <span className="text-2xl sm:text-3xl font-black" style={{ color: "#00D9FF" }}>
                  {stats?.countryCount ?? "–"}
                </span>
                <span className="text-xs sm:text-sm text-white font-medium">{t("hero.countries")}</span>
              </div>
              <span className="text-xl text-white/50 font-light">|</span>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl sm:text-3xl font-black" style={{ color: "#FF6B35" }}>
                  {stats?.worldQuizCount ?? "–"}
                </span>
                <span className="text-xs sm:text-sm text-white font-medium">{t("hero.worldQuizzes")}</span>
              </div>
              <span className="text-xl text-white/50 font-light">|</span>
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-xs sm:text-sm text-white font-medium">{t("hero.locations")}</span>
              </div>
            </div>
